#version 330

uniform sampler2D WaterNormalMap;

uniform float waterNormalMapDistance_W;
uniform float waterNormalMapDistance_H;
uniform float defaultHeight;

in vec2 vVaryingTexCoord0;

out vec4 vFragColor;

void main()
{
	// get the neighbouring y coordinates (height)
	float bottomHeight = texture(WaterNormalMap, vVaryingTexCoord0.st - vec2(waterNormalMapDistance_W, 0.0)).y;
	float topHeight = texture(WaterNormalMap, vVaryingTexCoord0.st + vec2(waterNormalMapDistance_W, 0.0)).y;
	float rightHeight = texture(WaterNormalMap, vVaryingTexCoord0.st + vec2(0.0, waterNormalMapDistance_H)).y;
	float leftHeight = texture(WaterNormalMap, vVaryingTexCoord0.st - vec2(0.0, waterNormalMapDistance_H)).y;

	// Create new normal vector, averaging the top/bottom height (x), the defaultHeight (y) and the average of left/right (z)
	vec3 Normal = normalize(vec3(bottomHeight - topHeight, defaultHeight, rightHeight - leftHeight));

	vFragColor = vec4(Normal, 1.0);
}